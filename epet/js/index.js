$(function(){
	...
	$('.showShop').click(function(){
		...
		$.post('/showShop',function(response){
			...
			for(i=0;i<arr.length;i++){
				$('.shopInfo .datalist').append('<tr><td><input type="checkbox"></td><td><input type="text" class="content1" value="'+arr[i].id+'"></td><td><input type="text" class="content2" value="'+arr[i].imgurl+'"></td><td><input type="text" class="content3" value="'+arr[i].name+'"></td><td><input type="text" class="content4" value="'+arr[i].produce+'"></td><td><input type="text" class="content5" value="'+arr[i].price+'"></td><td><input type="text" class="content6" value="'+arr[i].specification+'"></td></tr>');
			}
		})
	})
	...
	$('.searchInfo').click(function(){
		$('.shopInfo .datalist tr').remove;
		$('.logoInfo .datalist tr').remove;
		$('.search #searchInfo').remove();
		$("input[name='focusIn']").focus();
		$.post('/showShop',{id:'p1,p2'},function(response){
			var arr = response;
			var str = JSON.stringify(arr)
			var option ='';
			console.log(arr[0])

			$.each(arr[0], function(i, val) {  
    			option+='<option value="">'+i+'</option>'
			});
			
			$('.search').append('<select name="" id="searchInfo"></select>')
			$('#searchInfo').append(option)
