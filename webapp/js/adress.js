require(['config'],function(){
	require(['zhjquery','global'],function(){
		...
		$.post(erp.baseUrl +'/getaddress',{},function(response){
			...
			if( response.length > 0){
				...
				$('.acquiesce input[type=radio]').click(function(){
					...
					// 	//console.log(id)
					// 	$.post(erp.baseUrl + 'sort',{_id:id},function(response){					
					// 		//location.reload();							
					// 	})
					// }																																										
				})
				...
				$('.remove').click(function(){
					...
					var shamResponse=response.splice(idx,1)
						//console.log(shamResponse)
						// 把对应的数据在数据库中删除
					$.post(erp.baseUrl + '/deladdress',[{_id:shamResponse.id}],function(response){					
							location.reload();							
					})
				})
				...
				$('.edit').click(function(){
					...
					$('.save2').click(function(){
						...
						$.post(erp.baseUrl +'/updataadress',{"_id":id,"data":JSON.stringify({address:address_1,name:name,phone:phone})}
							,function(response){
		...
		$('.save1').click(function(){
			...
			$.post(erp.baseUrl +'/adress',{name:$.trim(name),phone:$.trim(phone),address:$.trim(address_1),
				//address:address_1,							
			},function(response){
				if(response.status){
					alert('设置失败')
